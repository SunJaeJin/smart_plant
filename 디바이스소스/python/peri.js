...
const SERVICE_UUID = 'ff02'; // 서비스식별자 (16bit 임의지정)
const CHARACTERISTIC_UUID = 'ff03'; // 특성식별자 (16bit 임의지정)

var analog_data = 0;
var nodename = "BLE-999"; // 블루투스 기기이름
var timerid = null;
...
ApproachCharacteristic.prototype.onReadRequest = function(offset, callback) {
    let data1 = Buffer.from(this._value.toString());
...
ApproachCharacteristic.prototype.onWriteRequest = function (data, offset, withoutResponse, callback) {
    if (data == 'A') { // 스마트폰 앱에서 ‘A’ 가 전송되어 오면
    console.log("블루투스> 데이터수신(write요청): " + data + ' (LED ON)');
    led.turnon( );
    }
    else if (data == 'a') { // 스마트폰 앱에서 ‘a’ 가 전송되어 오면
    console.log("블루투스> 데이터수신 (write요청): " + data + ' (LED OFF)');
    led.turnoff( );
    }
    else console.log("블루투스> 데이터수신:(" + data + ') A 또는 a 이외데이터');
...
bleno.on('accept', (addr) => {
    ...
    timerid = setInterval( ( ) => { bleno.updateRssi( (error, rssi) => {
