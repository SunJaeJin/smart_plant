    ...
    return (
        <ul style={{ listStyleType:"none", fontSize:"20px", width:"95%", maxHeight:"550px", overflowY:"auto", scrollbarColor:"gray", position:"relative", right:"27px"}}>
            {flowers && Array.isArray(flowers) && flowers.map((flower) => {
                ...
                return (
                    <li key={flower.id}>
                        <Link to={`/FlowerInfo/${flower.plantdata_ID}`} style={{ textDecoration: 'none', color: 'black', display: 'flex', alignItems: 'center', backgroundColor:"white", border:"1px solid lightgrey", borderRadius:"5px", marginBottom:"5px", marginRight:"5px"}}>
